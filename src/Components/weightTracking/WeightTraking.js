...
const WeightTracking = () => {
    ...
    const getUser = () => {
        axios.get('https://fit-at-home1.herokuapp.com/api/users/me', {
            ...
            console.log(res)
            if (res.status === 200) {
                setUserWeightHistory([...res.data.weightTracker,{weight:res.data.bmi.weight,date:res.data.lastVisit}])
            } else {
                toast(res.data)
            }
        })
    ...
    const addWeightToUser = () => {
        ...
        if (newWeight.weight !== "") {
            axios.put('https://fit-at-home1.herokuapp.com/api/users/updatingWeight', newWeight, {
                ...
                if (res.status === 200) {
                    setUserWeightHistory([...userWeightHistory,res.data.weightTracker])
                            ...
                            <Message>
                                ...
                                <Message.List>
                                    {userWeightHistory.map(ele => {
                                        return <MessageItem key={ele._id}>Weight: {ele.weight} At {ele.date || ""} {ele._id?<Button
